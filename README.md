...
Our group project is title Income prediction from the Adult data generated by UC Irvine Machine Learning Repository. 
The data tries to analyze individual income that exceeds $50K/yr. The extraction of the data was done by Barry Becker from the 1994 Census Data. 
Our group downloaded the CSV file and cleaned the file using Python Script and turned it into DataFrame. Prior to modeling the data, We normalized and standardized the data using spark. We split the data into a target and a feature array, and then used train_test_split to split the data into training and testing sets. We normalized the data with StandardScaler, and then fit and scaled the data.

The code was ran using Google Collab. 
