...
From the `Notebooks` folder in the repository, drag the files titled `first_model.ipynb`, `tuned_model.ipynb`, and (DYLAN’S CODE) to the `income_prediction` folder in your Google Drive.

From the `Resources` folder in the repository, drag the fileSSSS? titled `cleaned_df.csv`(AND DYLAN’S FILES) to the `income_prediction` folder in your Google Drive.
...
The same procedure can be followed for the tuned models by opening the `tuned_model.ipynb` and DYLAN’S FILE with Google Colaboratory.
